...
class TestScenarioUsersIndexRunner:
    def test_successful_fetch(self):
        """Test successfully fetching scenario users"""
        ...
        result = ScenarioUsersIndexRunner.run(mock_client, 123)

        assert result.success
        assert len(result.data) == 2
        assert result.data[0]["role"] == "scenario_owner"
        mock_client.session.get.assert_called_once_with("/scenarios/123/users")
    ...
    def test_failed_fetch(self):
        ...
        result = ScenarioUsersIndexRunner.run(mock_client, 999)

        assert not result.success
        assert "404" in result.errors[0]


class TestScenarioUsersCreateRunner:
    def test_successful_creation(self):
        """Test successfully creating scenario users"""
        mock_client = Mock(spec=BaseClient)
        mock_client.session = Mock()
        mock_response = Mock()
        mock_response.ok = True
        ...
        result = ScenarioUsersCreateRunner.run(mock_client, 123, users)

        assert result.success
        assert len(result.data) == 2
        mock_client.session.post.assert_called_once_with(
            "/scenarios/123/users", json={"scenario_users": users}
        )
    ...
    def test_empty_users_list(self):
        ...
        result = ScenarioUsersCreateRunner.run(mock_client, 123, [])
...
class TestScenarioUsersUpdateRunner:
    def test_successful_update(self):
        """Test successfully updating scenario user roles"""
        mock_client = Mock(spec=BaseClient)
        mock_client.session = Mock()
        mock_response = Mock()
        mock_response.ok = True
        ...
        result = ScenarioUsersUpdateRunner.run(mock_client, 123, users)
...
class TestScenarioUsersDestroyRunner:
    def test_successful_deletion(self):
        """Test successfully deleting scenario users"""
        mock_client = Mock(spec=BaseClient)
        mock_client.session = Mock()
        mock_response = Mock()
        mock_response.ok = True
        ...
        users = [{"user_id": 2}]
        result = ScenarioUsersDestroyRunner.run(mock_client, 123, users)
    ...
    def test_empty_users_list(self):
        ...
        result = ScenarioUsersDestroyRunner.run(mock_client, 123, [])
