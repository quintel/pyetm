...
Contains scenario metadata in rows:

| Field | Example Value |
|-------|---------------|
| title | My Scenario |
| session_id | 123456 |
| saved_scenario_id | 789 |
| area_code | nl |
| start_year | 2023 |
| end_year | 2050 |
...
| private | False |
| source | ETM |
| url | http://localhost:3000/api/v3/scenarios/1 |
| version | latest |
| created_at | 2026-05-21 12:00:00+00:00 |
| updated_at | 2026-05-21 12:00:00+00:00 |


### INPUTS Sheet

User values with scenarios as columns:

| Input Key | Scenario 1 | Scenario 2 | Scenario 3 |
|-----------|------------|------------|------------|
| number_of_energy_power_solar_pv_solar_radiation | 50000 | 75000 | 100000 |
| households_heater_district_heating_steam_hot_water_share | 25.0 | 30.0 | 35.0 |

### Hourly Curves Sheet

Time series data with 8760 rows (one per hour):

| Hour | total_demand | solar_pv | wind_turbine |
|------|--------------|----------|--------------|
| 0 | 12500.5 | 0.0 | 850.3 |
| 1 | 12100.2 | 0.0 | 920.1 |
| ... | ... | ... | ... |
| 8759 | 13200.8 | 0.0 | 780.5 |

### Annual Exports

Depends on export type. Example for `energy_flow`:

| From | To | Carrier | Present Value | Future Value |
|------|-----|---------|---------------|--------------|
| solar_pv | electricity_grid | electricity | 0.0 | 45000.0 |
| wind_turbine | electricity_grid | electricity | 12000.0 | 38000.0 |

## Troubleshooting

### Validation Warnings

When reading Excel files with `from_excel()`, pyetm validates configuration values and logs warnings for invalid entries.

#### Invalid Hourly Curve Entries

**Warning message:**
```
EXPORT_CONFIG: Invalid hourly curve entry 'electrcityyy'. Valid carriers: electricity, heat, hydrogen, methane. Valid curve names: merit_order, electricity_price, ...
```

**Cause:** The specified carrier or curve name doesn't exist in the Energy Transition Model.

**Solution:**
1. Check the spelling of the entry in your EXPORT_CONFIG sheet
2. Refer to the [Hourly Curves Options](#hourly-curves-options) section for valid values
3. Fix the entry in your Excel file and reload
...
#### Invalid Annual Export Types

**Warning message:**
```
EXPORT_CONFIG: Invalid annual export type 'sankeyyy'. Valid types: production_parameters, energy_flow, ...
```

**Cause:** The specified export type doesn't match any of the 7 available export types.

**Solution:**
1. Check the spelling in your EXPORT_CONFIG sheet
2. Refer to the [Annual Exports Options](#annual-exports-options) section for valid types
3. Correct the entry in your Excel file
...
If you specify multiple entries and some are invalid, pyetm will:
1. Filter out the invalid entries
2. Log a warning for each invalid entry
3. Continue processing with only the valid entries

**Example:**
```
| hourly_curves                    |
|----------------------------------|
| electricity, invalid, heat       |
```

Result: Exports `electricity` and `heat` curves only, logs warning about `invalid`

### Common Issues

#### No data exported despite configuration

**Cause:** All specified entries were invalid and filtered out.

**Solution:** Check the logs for validation warnings and correct all invalid entries.

#### Unexpected carriers/exports in output

**Cause:** When all entries are invalid, pyetm may default to exporting all available options.

**Solution:** Verify your EXPORT_CONFIG sheet has valid entries matching the documented options.
